...
\subsection{Timestamp Server}\label{timestamp-server}
...
\subsection{Proof-of-Work}\label{proof-of-work}
...
\begin{figure}
\centering
\includegraphics{proof-of-work.png}
\caption{}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{reclaiming-disk.png}
\caption{}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{combining-splitting.png}
\caption{}
\end{figure}
...
\DataTypeTok{double} \NormalTok{AttackerSuccessProbability(}\DataTypeTok{double} \NormalTok{q, }\DataTypeTok{int} \NormalTok{z)}
\NormalTok{\{}
    \DataTypeTok{double} \NormalTok{p = }\FloatTok{1.0} \NormalTok{- q;}
    \DataTypeTok{double} \NormalTok{lambda = z * (q / p);}
    \DataTypeTok{double} \NormalTok{sum = }\FloatTok{1.0}\NormalTok{;}
    \DataTypeTok{int} \NormalTok{i, k;}
    \ControlFlowTok{for} \NormalTok{(k = }\DecValTok{0}\NormalTok{; k <= z; k++)}
    \NormalTok{\{}
        \DataTypeTok{double} \NormalTok{poisson = exp(-lambda);}
        \ControlFlowTok{for} \NormalTok{(i = }\DecValTok{1}\NormalTok{; i <= k; i++)}
            \NormalTok{poisson *= lambda / i;}
        \NormalTok{sum -= poisson * (}\DecValTok{1} \NormalTok{- pow(q / p, z - k));}
    \NormalTok{\}}
